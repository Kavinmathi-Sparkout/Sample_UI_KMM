...
object string {
    ...
    const val DELIVERY_RETURN = "Delivery and Return"
    const val REVIEW = "Reviews"
    const val RETURN1 = "Free delivery for all orders above $200"
    const val RETURN2 = "30 days return policy"
    const val RETURN3 = "If your armpit smells, don't send it back"
    const val RICHARD = "Richard Hendricks"
    const val RICHARD_REVIEW = "The sneakers are incredibly lightweight and provide excellent support, making them perfect for my daily runs. The sole has great traction, which has helped me maintain my grip on slippery surfaces."
    const val  MONICA = "Monica Hall"
    const val MONICA_REVIEW = "The canvas material is durable and has held up well against daily wear and tear. The design is also classic and timeless, making it a versatile option that pairs well with a variety of outfits."
}